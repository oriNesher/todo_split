...
/// --- Color helpers: make primary slightly darker for "accent blocks"
...
/// Task with (de)serialization
/// root.currentId holds the id of the "current" task in that list
class Task {
  final String id;
  String title;
  bool done;
  DateTime? due;
  String notes;
  Task? parent; // rebuilt on load
  final List<Task> children;
  String? currentId; // only on root

  // NEW: completion reflection
...
class _SplitTodoPageState extends State<SplitTodoPage> {
  ...
  bool _isThirdLevel(Task t) => t.depth == 2; // 0=root, 1=child, 2=grandchild
  ...
  void _deleteTask(Task t) {
    // Root tasks (lists) are deleted via "Delete current list"
    if (t.parent == null) return;

    final root = _rootOf(t);
    _snapshot();

    // If CURRENT is inside this subtree, clear it
  ...
  Future<void> _promptCompletion(Task task) async {
    ...
    // Apply completion either way (save/skip)
    _snapshot();
    setState(() {
      task.done = true;
  ...
  // task ops
  void _addSingleSubtask(Task parent, String title) {
    final t = title.trim();
    if (t.isEmpty) return;
    _snapshot();
    setState(() =>
        parent.children.add(Task(id: _newId(), title: t, parent: parent)));
    _save();
  }

  void _showAddSingleDialog(Task task) {
    final c = TextEditingController();
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Add subtask to: ${task.title}'),
        content: TextField(
          controller: c,
          autofocus: true,
          decoration: const InputDecoration(
  ...
  void _splitTaskN(Task parent, List<String> titles,
    ...
    // אם לא מאפשרים יחידה יחידה – נדרוש לפחות 2 (ברירת מחדל כמו קודם)
  ...
  void _showSplitDialog(Task task) {
    ...
    int minCount = hasChildren ? 1 : 2; // אם כבר יש תתי־משימות – מותר גם 1
    int count = max(minCount, 2); // ברירת מחדל: 2 (או 1 אם מותר)
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                    ...
                    Switch(
                      ...
                      onChanged: (v) async {
                        if (v == true && done == false) {
                          // Ask reflection before toggling UI state
                          Navigator.pop(ctx);
                      ...
                      child: InputDecorator(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                                child:
                                    Text(due == null ? 'None' : fmtDate(due!))),
                            TextButton(
                              onPressed: () async {
                ...
                const SizedBox(height: 8),
                Wrap(
                    spacing: 8, runSpacing: 8, children: _buildPathChips(task)),
                const SizedBox(height: 20),
                Wrap(
                  spacing: 12,
                  runSpacing: 8,
                  children: [
                    ...
                    OutlinedButton.icon(
                      onPressed: () {
                        Navigator.pop(ctx);
                        _showAddSingleDialog(task);
                      },
                      icon: const Icon(Icons.add),
                      label: const Text('Add subtask'),
                    ),
                    OutlinedButton.icon(
                      onPressed: _isThirdLevel(task)
                          ? () {
                              _setCurrent(task);
                              Navigator.pop(ctx);
                            }
  ...
  Widget _taskTile(Task task) {
    ...
    final accent = primary.darken(0.1); // unified color for "current" + "goal"
          ...
          subtitle: Row(
            children: [
              ...
              if (task.children.isNotEmpty) ...[
                ...
                const SizedBox(width: 4),
                Text('${task.children.length} subtasks'),
              ],
            ],
          ),
          trailing: Wrap(
            ...
            children: [
              ...
              Tooltip(
                message: _isThirdLevel(task)
                    ? 'Mark as current'
                    : 'Only third-level tasks can be marked as CURRENT',
                child: IconButton(
                  onPressed:
                      _isThirdLevel(task) ? () => _setCurrent(task) : null,
                  icon: const Icon(Icons.my_location),
                ),
              ),
            ],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (ctx) => [
              const PopupMenuItem(
                  value: 'rename', child: Text('Rename current list')),
              const PopupMenuItem(
                  value: 'clear_current', child: Text('Clear current marker')),
              const PopupMenuItem(
                  value: 'reset', child: Text('Reset current list')),
              const PopupMenuItem(
                value: 'delete',
                child: Text('Delete current list',
                    style: TextStyle(color: Colors.red)),
              ),
            ],
          ),
        ],
        // Banner: same accent for CURRENT + Final goal
                    ...
                    children: [
                      Container(
                        ...
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.my_location, size: 16, color: onAccent),
      ...
      body: r == null
          ? const Center(child: Text('Create your first list to get started'))
          : ListView(
              padding: const EdgeInsets.fromLTRB(12, 12, 12, 96),
              children: [
                // Final goal strip with the SAME accent as CURRENT
                Container(
